package io.kommunicate.data.account.register;
...
import io.kommunicate.data.conversation.ConversationIntentService;
import io.kommunicate.notification.NotificationChannels;
import io.kommunicate.data.api.authentication.JWT;
...
public class RegisterUserClientService extends MobiComKitClientService {
    ...
    public RegisterUserClientService(Context context) {
        ...
        KommunicateService.initWithContext(context);
    ...
    public RegistrationResponse createAccount(User user) throws Exception {
        ...
        if (TextUtils.isEmpty(user.getUserId())) {
            throw new KommunicateException("userId cannot be empty");
        }

        if (!user.isValidUserId()) {
            throw new KommunicateException("Invalid userId. Spacing and set of special characters ^!$%&*:(), are not accepted. \nOnly english language characters are accepted");
        }
        ...
        if (registrationResponse.isRegistrationSuccess()) {
            ...
            KommunicateClient.getInstance(context).skipDeletedGroups(user.isSkipDeletedGroups()).hideActionMessages(user.isHideActionMessages());
            ...
            if (!TextUtils.isEmpty(user.getNotificationSoundFilePath())) {
                KmChat.getInstance(context).setCustomNotificationSound(user.getNotificationSoundFilePath());
            }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                KmChat.getInstance(context).setNotificationChannelVersion(NotificationChannels.NOTIFICATION_CHANNEL_VERSION - 1);
                new NotificationChannels(context, KmChat.getInstance(context).getCustomNotificationSound()).prepareNotificationChannels();
            }
            KommunicateClient.getInstance(context).setChatDisabled(contact.isChatForUserDisabled());
            ...
            intent.putExtra(KmMqttIntentService.CONNECTED_PUBLISH, true);
