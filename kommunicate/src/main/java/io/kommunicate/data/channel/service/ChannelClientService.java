...
import io.kommunicate.models.MultipleChannelFeedApiResponse;
...
public class ChannelClientService extends MobiComKitClientService {
    ...
    public static ChannelClientService getInstance(Context context) {
        if (channelClientService == null) {
            channelClientService = new ChannelClientService(KommunicateService.getContext(context));
        }
    ...
    public ApiResponse addMemberToContactGroupOfType(String contactGroupId, String groupType,
        ...
        if (!TextUtils.isEmpty(contactGroupId) && !TextUtils.isEmpty(groupType) &&
            ...
            KmAddMemberOfGroupType kmAddMemberOfGroupType = new
                    KmAddMemberOfGroupType();
            kmAddMemberOfGroupType.setGroupMemberList(contactGroupMemberList);
            kmAddMemberOfGroupType.setType(groupType);
            String jsonFromObject = GsonUtils.getJsonFromObject(kmAddMemberOfGroupType,
                    KmAddMemberOfGroupType.class);
            Utils.printLog(context, TAG, "Sending json:" + jsonFromObject);
            try {
                response = httpRequestUtils.postData(url, "application/json", "application/json",
                        jsonFromObject);
