package io.kommunicate.data.channel.service;
...
import io.kommunicate.data.async.AlGetPeopleTask;
import io.kommunicate.models.ChannelInfo;
import io.kommunicate.data.cache.MessageSearchCache;
import io.kommunicate.data.channel.database.ChannelDatabaseService;
...
import io.kommunicate.models.feed.AlResponse;
...
import io.kommunicate.models.feed.ChannelFeedListResponse;
import io.kommunicate.models.feed.ChannelUsersFeed;
import io.kommunicate.models.feed.GroupInfoUpdate;
import io.kommunicate.models.feed.sync.SyncChannelFeed;
...
public class ChannelService {
    ...
    private ChannelService(Context context) {
        ...
        channelClientService = ChannelClientService.getInstance(KommunicateService.getContext(context));
        channelDatabaseService = ChannelDatabaseService.getInstance(KommunicateService.getContext(context));
        userService = UserService.getInstance(KommunicateService.getContext(context));
        baseContactService = new AppContactService(KommunicateService.getContext(context));
    ...
    public synchronized static ChannelService getInstance(Context context) {
        if (channelService == null) {
            channelService = new ChannelService(KommunicateService.getContext(context));
        }
    ...
    public void processChannelFeedForSync(ChannelFeed channelFeed) {
        if (channelFeed != null) {
            ...
            if (channel.isDeleted() && KommunicateClient.getInstance(context).isSkipDeletedGroups()) {
