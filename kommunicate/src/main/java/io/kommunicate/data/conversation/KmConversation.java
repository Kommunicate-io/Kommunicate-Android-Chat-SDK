...
import io.kommunicate.callbacks.MediaDownloadProgressHandler;
...
public class KmConversation {
    ...
    public static void downloadMessage(Context context, Message message, MediaDownloadProgressHandler handler) {
        ...
        if (!message.hasAttachment()) {
            e = new KommunicateException("Message does not have Attachment");
            handler.onProgressUpdate(0, e);
            handler.onCompleted(null, e);
        } else if (message.isAttachmentDownloaded()) {
            e = new KommunicateException("Attachment for the message already downloaded");
            handler.onProgressUpdate(0, e);
            handler.onCompleted(null, e);
        } else {
    ...
    public static boolean isMessageStatusPublished(Context context, String pairedMessageKey, Short status) {
        KmMqttService kmMqttService = KmMqttService.getInstance(context);

        if (!TextUtils.isEmpty(pairedMessageKey) && kmMqttService.isConnected()) {
            kmMqttService.publishMessageStatus(MESSAGE_STATUS_TOPIC, MobiComUserPreference.getInstance(context).getUserId() + "," + pairedMessageKey + "," + status);
            return true;
        }
        return false;
    }
