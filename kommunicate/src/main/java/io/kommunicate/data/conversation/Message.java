...
public class Message extends JsonMarker {
    ...
    public boolean isIgnoreMessageAdding(Context context) {
        if (KommunicateClient.getInstance(context).isSubGroupEnabled() && MobiComUserPreference.getInstance(context).getParentGroupKey() != null || !TextUtils.isEmpty(MobiComUserPreference.getInstance(context).getCategoryName())) {
            ...
            return (subGroupFlag || categoryFlag || KommunicateClient.getInstance(context).isSubGroupEnabled() || !TextUtils.isEmpty(MobiComUserPreference.getInstance(context).getCategoryName()));
        }
        return ((KommunicateClient.getInstance(context).isActionMessagesHidden() && isActionMessage()) || hasHideKey());
    }
