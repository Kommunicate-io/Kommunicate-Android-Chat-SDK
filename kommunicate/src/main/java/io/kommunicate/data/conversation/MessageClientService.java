...
import io.kommunicate.data.conversation.schedule.ScheduledMessageUtil;
...
import io.kommunicate.models.feed.MessageResponse;
import io.kommunicate.models.feed.sync.SmsSyncRequest;
import io.kommunicate.models.feed.sync.SyncMessageFeed;
...
public class MessageClientService extends MobiComKitClientService {
    ...
    public String getMessageDeleteForAllResponse(String messageKey, boolean deleteForAll) throws Exception {
        if (TextUtils.isEmpty(messageKey)) {
            throw new KommunicateException("Message key cannot be empty");
        }
    ...
    public void processMessage(Message message, Handler handler, String userDisplayName) throws Exception {
        ...
        try {
            ...
            if (handler != null) {
                ...
                msg.sendToTarget();
                //handler.onCompleted(new KommunicateException("Error uploading file"));
            }
        }
    ...
    public String getMessages(Contact contact, Channel channel, Long startTime, Long endTime, Integer conversationId, boolean isSkipRead) throws UnsupportedEncodingException {
        ...
        if (contact == null && channel == null) {
            params = "startIndex=0&mainPageSize=" + KommunicateClient.getInstance(context).getFetchConversationListMainPageSize() + "&";
        }
        ...
        params = params + "&" + "deletedGroupIncluded=" + String.valueOf(!KommunicateClient.getInstance(context).isSkipDeletedGroups());
