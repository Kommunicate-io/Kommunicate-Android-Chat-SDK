...
import io.kommunicate.callbacks.MediaUploadProgressHandler;
...
import io.kommunicate.data.json.AnnotationExclusionStrategy;
import io.kommunicate.data.json.ArrayAdapterFactory;
...
import io.kommunicate.models.feed.sync.SyncUserDetailsResponse;
...
public class MobiComConversationService {
    ...
    public MobiComConversationService(Context context) {
        ...
        this.isHideActionMessage = KommunicateClient.getInstance(context).isActionMessagesHidden();
    }
    ...
    public void sendMessage(Message message, MediaUploadProgressHandler handler) {
        ...
        if (!message.hasAttachment()) {
            e = new KommunicateException("Message does not have any attachment");
            if (handler != null) {
    ...
    public synchronized List<Message> getLatestMessagesGroupByPeople(Long createdAt, String searchString, Integer parentGroupKey) {
        ...
        if (!KommunicateClient.getInstance(context).wasServerCallDoneBefore(null, null, null) || createdAt != null && createdAt != 0) {
    ...
    public synchronized List<Message> getMessages(Long startTime, Long endTime, Contact contact, Channel channel, Integer conversationId, boolean isSkipRead, boolean isForSearch) {
        ...
        if (isServerCallNotRequired && (!cachedMessageList.isEmpty() &&
                KommunicateClient.getInstance(context).wasServerCallDoneBefore(contact, channel, conversationId)
                || (contact == null && channel == null && cachedMessageList.isEmpty() && KommunicateClient.getInstance(context).wasServerCallDoneBefore(contact, channel, conversationId)))) {
        ...
        KommunicateClient.getInstance(context).updateServerCallDoneStatus(contact, channel, conversationId);
