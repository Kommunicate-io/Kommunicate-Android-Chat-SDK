package io.kommunicate.data.conversation.database;
...
import io.kommunicate.data.database.MobiComDatabaseHelper;
...
public class MessageDatabaseService {
    ...
    public MessageDatabaseService(Context context) {
        ...
        hideActionMessages = KommunicateClient.getInstance(context).isActionMessagesHidden();
        skipDeletedGroups = KommunicateClient.getInstance(context).isSkipDeletedGroups();
    }
    ...
    public long createSingleMessage(final Message message) {
        ...
        KommunicateClient kommunicateClient = KommunicateClient.getInstance(context);
        long id = -1;
        boolean duplicateCheck = true;
        long minCreatedAt = kommunicateClient.getMinCreatedAtTime();
        long maxCreatedAt = kommunicateClient.getMaxCreatedAtTime();

        if (message.getCreatedAtTime() < minCreatedAt) {
            duplicateCheck = false;
            kommunicateClient.setMinCreatedAtTime(message.getCreatedAtTime());
        }
        if (message.getCreatedAtTime() > maxCreatedAt) {
            duplicateCheck = false;
            kommunicateClient.setMaxCreatedAtTime(message.getCreatedAtTime());
        }
