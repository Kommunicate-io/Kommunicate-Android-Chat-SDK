...
public class AlPrefSettings {
    ...
    private AlPrefSettings(Context context) {
        this.sharedPreferences = new SecureSharedPreferences(AL_PREF_SETTING_KEY, KommunicateService.getContext(context));
    }

    public static AlPrefSettings getInstance(Context context) {
        if (alPrefSettings == null) {
            alPrefSettings = new AlPrefSettings(KommunicateService.getContext(context));
        }
