...
public class NotificationChannels {
    ...
    public void prepareNotificationChannels() {
        if (KmChat.getInstance(context).getNotificationChannelVersion() < NOTIFICATION_CHANNEL_VERSION) {
            ...
            if (isAppChannelCreated()) {
                KmChat.getInstance(context).setCustomNotificationSound(null);
                soundFilePath = null;
            ...
            KmChat.getInstance(context).setNotificationChannelVersion(NOTIFICATION_CHANNEL_VERSION);
        }
    }
    ...
    private synchronized void createNotificationChannel() {
        ...
        if (mNotificationManager != null && mNotificationManager.getNotificationChannel(MobiComKitConstants.AL_PUSH_NOTIFICATION) == null) {
            ...
            mChannel.setShowBadge(KommunicateClient.getInstance(context).isUnreadCountBadgeEnabled());

            if (KommunicateClient.getInstance(context).getVibrationOnNotification()) {
    ...
    private synchronized void createAppNotificationChannel() throws KommunicateException {
        ...
        if (mNotificationManager != null && mNotificationManager.getNotificationChannel(MobiComKitConstants.AL_APP_NOTIFICATION) == null) {
            ...
            if (TextUtils.isEmpty(soundFilePath)) {
                throw new KommunicateException("Custom sound path is required to create App notification channel. " +
                        "Please set a sound path using KmChat.getInstance(context).setCustomNotificationSound(your-sound-file-path)");
            }
