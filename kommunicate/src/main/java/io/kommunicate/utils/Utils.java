...
import io.kommunicate.data.file.ALFileProvider;
...
public class Utils {
    ...
    public static boolean isInternetAvailable(Context context) {
        ConnectivityManager cm =
                (ConnectivityManager) KommunicateService.getContext(context).getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public static String getMetaDataValue(Context context, String metaDataName) {
        try {
            PackageManager packageManager = KommunicateService.getContext(context).getPackageManager();
            ApplicationInfo ai = packageManager.getApplicationInfo(KommunicateService.getContext(context).getPackageName(), PackageManager.GET_META_DATA);
            if (ai.metaData != null) {
    ...
    public static boolean isAutomaticTimeEnabled(Context context, boolean isTimeZone) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            return Settings.Global.getInt(KommunicateService.getContext(context).getContentResolver(), (isTimeZone ? Settings.Global.AUTO_TIME_ZONE : Settings.Global.AUTO_TIME), 0) == 1;
        } else {
            return android.provider.Settings.System.getInt(KommunicateService.getContext(context).getContentResolver(), (isTimeZone ? Settings.System.AUTO_TIME_ZONE : Settings.System.AUTO_TIME), 0) == 1;
        }
    }

    public static int getLauncherIcon(Context context) {
        try {
            ApplicationInfo ai = KommunicateService.getContext(context).getPackageManager().getApplicationInfo(KommunicateService.getContext(context).getPackageName(), PackageManager.GET_META_DATA);
            return ai.icon;
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
            return 0;
    ...
    public static String getMetaDataValueForReceiver(Context context, String componentName, String metaDataName) {
        try {
            ActivityInfo ai = KommunicateService.getContext(context).getPackageManager().getReceiverInfo(new ComponentName(KommunicateService.getContext(context), componentName), PackageManager.GET_META_DATA);
            return ai.metaData.getString(metaDataName);
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
            return null;
    ...
    public static boolean isDebugBuild(Context context) {
        return (0 != (KommunicateService.getContext(context).getApplicationInfo().flags & ApplicationInfo.FLAG_DEBUGGABLE));
    }
    ...
    public static Uri getTextLogFileUri(Context context) {
        try {
            ...
            if (hasNougat()) {
                return ALFileProvider.getUriForFile(KommunicateService.getContext(context), getMetaDataValue(context, "com.package.name") + ".kmChat.provider", textLogFile);
            }
    ...
    public static String getString(Context context, int resId) {
        return KommunicateService.getContext(context).getString(resId);
    }

    public static int getColor(Context context, int resId) {
        return KommunicateService.getContext(context).getResources().getColor(resId);
    }

    public static String getPackageName(Context context) {
        return KommunicateService.getContext(context).getPackageName();
    }
}
